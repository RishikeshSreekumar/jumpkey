...
describe("looksLikeId", () => {
  ...
  it.each(["workflow", "asset-editor", "settings2", "issues", "AcmeHQ", "v2"])("rejects %s", (s) => {
    expect(looksLikeId(s)).toBe(false);
  });
});

describe("suggestName", () => {
  const url = "https://api.github.com/AcmeHQ/repos/issues/42?page=3#/Asset-Editors/9";
  ...
  it("names path segments after the singularized route word before them", () => {
    ...
    expect(suggestName(tokens, at("repos"))).toBe("acmehq");
  });

  it("falls back to id when the previous segment is itself an id or absent", () => {
    expect(suggestName(tokens, at("AcmeHQ"))).toBe("id");
...
describe("suggestVariables / buildTemplate / alignTemplate", () => {
  const url = "https://example.com/workflow/34173cdf-655a-4d39-8ef0-e81c7b2beb8e/asset-editor?v=12";
  const tokens = splitUrl(url)!;

  it("prefills id-like slots, never host labels", () => {
    const sel = suggestVariables(tokens);
    expect(buildTemplate(tokens, sel)).toBe("https://example.com/workflow/{workflow}/asset-editor?v={v}");
  });
  ...
  it("refuses templates that no longer match slot-for-slot", () => {
    expect(alignTemplate(tokens, "https://example.com/workflow/{id}")).toBeNull();
    expect(alignTemplate(tokens, "https://example.com/workflow/x{id}/asset-editor?v=12")).toBeNull();
...
describe("nameFromTitle", () => {
  it("takes the first chunk before a separator", () => {
    expect(nameFromTitle("Fix login bug · Issue #42 · AcmeHQ/app")).toBe("Fix login bug");
    expect(nameFromTitle("Dashboard - Acme")).toBe("Dashboard");
    expect(nameFromTitle("well-known | site")).toBe("well-known");
    expect(nameFromTitle(undefined)).toBe("");
  });
});

describe("alignTemplate with variable specs", () => {
  it("aligns variables that carry a default or rest marker", () => {
    const tokens = splitUrl("https://x.com/orders/5531?q=hello")!;
    expect(alignTemplate(tokens, "https://x.com/orders/{id=1}?q={q*}")).toEqual(
      new Map([
        [tokens.findIndex((t) => t.text === "5531"), "id"],
        [tokens.findIndex((t) => t.text === "hello"), "q"],
      ]),
    );
  });
});
