...
/** Exact match on Keyword or Alias. Never matches by prefix. */
...
/**
 * Filter for the Launcher list. Uses only the first token of `input`.
 * Ranking: keyword prefix, then alias prefix, then name substring.
 * With empty input, most recently used Commands come first; the rest keep
 * their stored order.
 */
...
/** Closest Keyword or Alias to a mistyped one, for "Did you mean". Undefined if nothing is close. */
