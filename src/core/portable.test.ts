...
describe("pattern round-trip", () => {
  it("exports and imports a command's pattern", () => {
    const c: Command = { id: "1", keyword: "jira", name: "Jira", template: "https://x.com/{t}", pattern: "[A-Z]+-\\d+", openMode: "foreground-tab", enabled: true, createdAt: 0, updatedAt: 0 };
    const parsed = parseImport(exportCommands([c]));
    expect(parsed).toMatchObject({ ok: true, commands: [{ keyword: "jira", pattern: "[A-Z]+-\\d+" }] });
  });
});
