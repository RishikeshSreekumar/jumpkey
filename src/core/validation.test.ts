...
describe("normalizeTemplate", () => {
  it("prefixes https:// when scheme is absent", () => {
    expect(normalizeTemplate("example.com/workflow/{id}")).toBe("https://example.com/workflow/{id}");
  });
...
describe("pattern", () => {
  const base = { keyword: "jira", name: "Jira", template: "https://x.atlassian.net/browse/{t}", openMode: "foreground-tab" as const };

  it("accepts a valid pattern and trims it", () => {
    expect(normalizeDraft({ ...base, pattern: "  [A-Z]+-\\d+ " }).pattern).toBe("[A-Z]+-\\d+");
    expect(validateDraft({ ...base, pattern: "[A-Z]+-\\d+" }, []).errors).toEqual({});
  });

  it("drops an empty pattern", () => {
    expect(normalizeDraft({ ...base, pattern: "  " })).not.toHaveProperty("pattern");
  });

  it("rejects invalid regexes and ones that match nothing typed", () => {
    expect(validateDraft({ ...base, pattern: "([" }, []).errors.pattern).toBeDefined();
    expect(validateDraft({ ...base, pattern: ".*" }, []).errors.pattern).toBeDefined();
  });

  it("warns when the template cannot take a single value", () => {
    const v = validateDraft({ ...base, template: "https://x.com/{a}/{b}", pattern: "\\d+" }, []);
    expect(v.warnings.some((w) => w.includes("exactly one required"))).toBe(true);
  });

  it("rejects a default that cannot render", () => {
    expect(validateDraft({ ...base, template: "https://{env=a.b}.x.com/{t}" }, []).errors.template).toMatch(/Default value/);
  });
});
